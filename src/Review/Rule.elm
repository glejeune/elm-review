...
accumulateList2 : List (Node a -> context -> ( List Error, context )) -> List (Node a) -> ( List Error, context ) -> ( List Error, context )
accumulateList2 visitors nodes ( previousErrors, previousContext ) =
    List.foldl
        (\node -> accumulate2 visitors node)
...
{-| Concatenate the errors of the previous step and of the last step, and take the last step's context.
-}
accumulate2 : List (node -> context -> ( List Error, context )) -> node -> ( List Error, context ) -> ( List Error, context )
accumulate2 visitors node ( previousErrors, previousContext ) =
    List.foldl
        (\visitor -> accumulate (visitor node))
        ( previousErrors, previousContext )
        visitors
